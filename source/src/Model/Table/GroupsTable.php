...
class GroupsTable extends Table
{
    ...
    public function getOne($conditions = [], $order = [], $contain = []){
        $this->_one = true;
        $this->_datas = $this->find()->select(['groups.id'])->contain($contain)->where($conditions)->order($order)->first();
        return $this;
    }

    public function getMany($conditions = [], $order = [], $contain = [], $limit = 20) {
        $this->_one = false;
        $table = $this->find()->select(['groups.id'])->contain($contain)->where($conditions);
